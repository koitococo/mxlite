...
impl FromStr for DiscoveryResponse {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> { serde_json::from_str::<DiscoveryResponse>(s) }
}
...
impl FromStr for DiscoveryRequest {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> { serde_json::from_str::<DiscoveryRequest>(s) }
}
