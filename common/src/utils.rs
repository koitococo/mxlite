...
pub async fn digest_for_file(
    path: &str,
    mut hasher: Box<dyn DynDigest + Send + Unpin>,
    ...
    Ok(lower::encode_str(&hash, buf.as_mut_slice())
        .map_err(HashError::Base16Error)?
        .to_string())
}

/// SHA1 hash for a file
pub async fn sha1_for_file(path: &str) -> Result<String, HashError> {
    digest_for_file(path, Box::new(<sha1::Sha1 as sha1::Digest>::new())).await
}

/// SHA2-256 hash for a file
pub async fn sha256_for_file(path: &str) -> Result<String, HashError> {
    digest_for_file(path, Box::new(<sha2::Sha256 as sha2::Digest>::new())).await
}

/// SHA3-512 hash for a file
pub async fn sha512_for_file(path: &str) -> Result<String, HashError> {
    digest_for_file(path, Box::new(<sha3::Sha3_512 as sha3::Digest>::new())).await
}

pub async fn digests_for_file(
    path: &str,
    mut hashers: Vec<Box<dyn DynDigest + Send + Unpin>>,
    ...
    for hasher in &mut hashers {
        ...
        let mut buf = vec![0u8; hash.len() * 2];
        hashes.push(
            lower::encode_str(&hash, buf.as_mut_slice())
                .map_err(HashError::Base16Error)?
                .to_string(),
        );
    }
