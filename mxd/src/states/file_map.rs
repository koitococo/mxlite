...
impl FileMapStorage {
    ...
    pub(crate) fn get_all_files(&self) -> Vec<String> {
        self.0.list()
    }

    pub(crate) fn del_file_map(&self, publish_name: &String) {
        self.0.remove(publish_name);
    }
    ...
    pub(crate) async fn get_file_with_optional_props(
        ...
        if let Some(file_map) = self.0.get(publish_name) {
            ...
            if ensure_sha1 || ensure_sha256 || ensure_sha512 {
                ...
                if let Ok((sha1, sha256, sha512)) =
                    utils::sha_for_file(&new_inner.file_path, calc_sha1, calc_sha256, calc_sha512)
                        .await
                {
                    if calc_sha1 {
                        new_inner.sha1 = sha1;
