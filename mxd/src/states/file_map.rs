...
impl FileMapStorage {
  ...
  pub(crate) async fn add_file_map(&self, file_path: String, publish_name: String) -> Result<(), String> {
  ...
  pub(crate) fn get_all_files(&self) -> Vec<String> { self.0.list() }

  pub(crate) fn del_file_map(&self, publish_name: &String) { self.0.remove(publish_name); }
